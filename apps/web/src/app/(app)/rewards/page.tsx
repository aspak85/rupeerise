...
export default function RewardsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const s = await api<Status>("/rewards/status");
      setStatus(s);
    } catch {}
  }, []);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto w-full">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {(status?.spin.enabled ?? true) && (
          <SpinWheel
            available={status?.spin.available ?? true}
            claimed={status?.spin.claimedAmount ?? 0}
            prizes={status?.spin.prizes ?? [5, 10, 15, 20, 25, 35, 50, 100]}
            onWin={(amount) => {
        ...
        {(status?.scratch.enabled ?? true) && (
          <ScratchCard
            available={status?.scratch.available ?? true}
            claimed={status?.scratch.claimedAmount ?? 0}
            maxPrize={status?.scratch.maxPrize ?? 200}
            onWin={(amount) => {
        ...
        {status && !status.spin.enabled && !status.scratch.enabled && (
...
function SpinWheel({
  ...
  const handleSpin = async () => {
    ...
    try {
      ...
      // Compute final rotation: 6 full turns + landing on prize index.
      // The pointer is at top (12 o'clock). Each segment centered at i * segAngle + segAngle/2 starting at -90°.
      const target = 360 * 6 - (r.index * segAngle + segAngle / 2);
